...
{
	if (!UnitManager.inst)
		UnitManager.inst = new UnitManager();
...
{
	var setlDecDispDigt = theApp.master.getItemValue(symbol, 'o_setl_dec_disp_digt');
...
{
	var trgtDecDispDigt = theApp.master.getItemValue(symbol, 'o_trgt_dec_disp_digt');
...
UnitManager.getHogaUnit = function(unit, curCd, prc, HogaRangeArray, isMinus, nLenBelowDigitArray)
{
	...
	for (var index = 0; index < hogaUnits.length; ++index)
	{
		...
		if (hogaUnit.setl_cur_cd == curCd)
		{
			...
			{
				var highLimit = isMinus == true ? prc <= hogaUnit.prc_high_limt : prc < hogaUnit.prc_high_limt;
...
UnitManager.getMaxLenBelowDigit = function(curCd)
{
	...
	if (nMaxLenBelowDigit)
		return nMaxLenBelowDigit;
	return 0;	
};
...
UnitManager.fixToOrdUnitQtyDigit = function(symbol, Qty)
{
	...
	if(nPointPos >= 0)
		nOrdUnitQtyDigit = strOrdUnitQty.length - nPointPos - 1;		
	...
	var nDigit = 0;
	if(nPointPos >= 0)
		nDigit = strQty.length - nPointPos - 1;
...
UnitManager.fixToTargetDisplayDigit = function(symbol, Qty)
{
	...
	if(nTrgtDispDigit > nDigit)
		strQty = Qty.toFixed(nTrgtDispDigit).toString();		
